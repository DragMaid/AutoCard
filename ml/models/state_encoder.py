...
class GameStateEncoder(nn.Module):
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        hand_cards = state[:, p_offset:h_offset].reshape(batch_size, self.max_hand_cards, self.card_dim)
        
        # Board cards
        b_offset = h_offset + (self.max_board_cards * self.card_dim)
        board_cards = state[:, h_offset:b_offset].reshape(batch_size, self.max_board_cards, self.card_dim)
