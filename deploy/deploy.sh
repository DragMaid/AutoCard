...
# Through Traefik on this box, so a route missing its labels fails the release
# as surely as a dead container. -k because the certificate may still be
# Traefik's self-signed default while Let's Encrypt has yet to issue one.
through_traefik() {
    curl -fsSk -o /dev/null --max-time 10 \
        --resolve "${site_domain}:443:127.0.0.1" "https://${site_domain}$1"
}
...
healthy() {
    for _ in $(seq 1 180); do
        if container_healthy engine && container_healthy web &&
           through_traefik /relay-health && through_traefik /; then
            return 0
        fi
        sleep 2
    done
